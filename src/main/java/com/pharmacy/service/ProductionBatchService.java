...
public class ProductionBatchService {
    ...
    public ProductionBatchDto toDto(ProductionBatch pb) {
        ...
        if (pb.getMaterialsConsumed() != null) {
            dto.setMaterialsConsumed(pb.getMaterialsConsumed().stream()
    ...
    public ProductionBatch fromDto(ProductionBatchDto dto) {
        ...
        pb.setExpectedQuantity(dto.getExpectedQuantity() != null ? dto.getExpectedQuantity() : recipe.getOutputQuantity());
    ...
    public ProductionBatchDto create(ProductionBatchDto dto) {
        ...
        if (dto.getMaterialsConsumed() != null && !dto.getMaterialsConsumed().isEmpty()) {
            List<ProductionBatchMaterial> materials = dto.getMaterialsConsumed().stream()
                .map(matDto -> {
                    ProductionBatchMaterial pbm = new ProductionBatchMaterial();
                    pbm.setProductionBatch(savedBatch);

                    Ingredient ingredient = ingredientRepository.findById(matDto.getIngredientId())
                        .orElseThrow(() -> new RuntimeException("Ingredient not found: " + matDto.getIngredientId()));
            ...
            List<ProductionBatchMaterial> materials = recipe.getIngredients().stream()
                .map(ri -> {
                    ProductionBatchMaterial pbm = new ProductionBatchMaterial();
                    pbm.setProductionBatch(savedBatch);
    ...
    public void delete(Long id) {
        ProductionBatch pb = productionBatchRepository.findById(id)
            .orElseThrow(() -> new RuntimeException("Production batch not found: " + id));
    ...
    public List<ProductionBatchDto> searchProductionBatches(
            String businessLocation, String status, Boolean isFinalized,
            Long recipeId, Long productId, LocalDateTime startDate, LocalDateTime endDate) {
        return productionBatchRepository.searchProductionBatches(
    ...
    public ProductionBatchDto finalize(Long id, String finalizedBy) {
        ...
        if (pb.getMaterialsConsumed() != null) {
            pb.getMaterialsConsumed().forEach(material -> {
                Ingredient ingredient = material.getIngredient();
                BigDecimal newStock = ingredient.getCurrentStock().subtract(material.getQuantityUsed());
    ...
    private void calculateProductionCosts(ProductionBatch pb) {
        ...
        if (pb.getMaterialsConsumed() != null) {
            ingredientCost = pb.getMaterialsConsumed().stream()
        ...
        BigDecimal productionCost = (recipe.getFixedProductionCost() != null ? recipe.getFixedProductionCost() : BigDecimal.ZERO)
    ...
    private String generateReferenceNumber() {
        ...
        List<ProductionBatch> todayBatches = productionBatchRepository.findRecentBatches(
        ...
        if (!todayBatches.isEmpty()) {
            ...
            nextNumber = todayBatches.stream()
