...
public class RecipeController {
    ...
    public ResponseEntity<List<RecipeDto>> listAll(
            @Parameter(description = "Filter for active recipes only")
            @RequestParam(required = false) Boolean activeOnly
    ) {
        List<RecipeDto> recipes = activeOnly != null && activeOnly
            ? recipeService.listAllActive()
    ...
    @Operation(
        ...
        description = "Search recipes using multiple filter criteria including name, category, and product"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<List<RecipeDto>> search(
            @Parameter(description = "Search query for recipe name") @RequestParam(required = false) String q,
            @Parameter(description = "Filter by category") @RequestParam(required = false) String category,
            @Parameter(description = "Filter by sub-category") @RequestParam(required = false) String subCategory,
            @Parameter(description = "Filter by status (ACTIVE, DRAFT, ARCHIVED)") @RequestParam(required = false) String status,
        ...
        List<RecipeDto> recipes = recipeService.searchRecipes(
            category, subCategory, status, productId, q
        );
        return ResponseEntity.ok(recipes);
    }
    ...
    public ResponseEntity<List<RecipeDto>> getByCategory(
            @Parameter(description = "Category name", example = "Packaging Material") @PathVariable String category) {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "Invalid input or recipe code already exists")
    })
    @PostMapping
