...
/**
 * Tracks actual material/ingredient consumption for each production batch This provides detailed
 * traceability for GMP compliance
 */
@Entity
@Table(
  ...
  protected void calculateVariance() {
    if (quantityUsed != null && quantityRequired != null) {
      ...
      if (quantityRequired.compareTo(BigDecimal.ZERO) > 0) {
        variancePercent =
            variance
                .divide(quantityRequired, 4, java.math.RoundingMode.HALF_UP)
